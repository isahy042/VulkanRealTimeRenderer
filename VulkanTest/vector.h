...
/// A \f$4 \times 4\f$ matrix for expressing affine and perspective
/// transformations
/**
...
template <typename T>
inline Mat44<T>::Mat44(const T mat[4][4]) { memcpy(m, mat, 16 * sizeof(T)); }
...
template <typename T>
inline const Mat44<T>& operator/=(Mat44<T>& m, T a) { return (m *= T(1) / a); }
...
template <typename T>
inline Mat44<T> operator*(T a, const Mat44<T>& m) { return m * a; }
...
template <typename T>
inline Mat44<T> operator/(const Mat44<T>& m, T a) { return m * (T(1) / a); }
...
template <typename T>
inline Mat44<T> inverse(const Mat44<T>& m) { return adjugate(m) / determinant(m); }
...
Mat44<T> axisAngle(const Vec<3, S>& v, S a)
{
    ...
    return {
        {omc * v.x * v.x + ct, omc * v.x * v.y + st * v.z, omc * v.x * v.z - st * v.y, 0},
...
{
    return Color3f(std::max(c.r, 0.0f), std::max(c.g, 0.0f), std::max(c.b, 0.0f));
}
...
inline float luminance(const Color3f& c) { return c[0] * 0.212671f + c[1] * 0.715160f + c[2] * 0.072169f; }
